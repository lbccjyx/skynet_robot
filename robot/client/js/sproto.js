//https://github.com/simonxlg/sproto-js/blob/master/sproto.js
...
export var Sproto = {
    ...
    new: function(data) {
        ...
        // 新增：从解析后的协议创建 sproto 实例
        function createFromParsedProtocols(protocols) {
            try {
                // 直接使用外部的 sproto 对象，而不是创建新的局部变量
                // var localSproto = {};
                // var localResult = {};
                // var localSession = [];
                // var localBuf = [];
                ...
                for (var key in protocols) {
                    ...
                    if (protocol.type === 'struct') {
                        var type = {
                            name: protocol.name,
                            f: [],
                            n: protocol.fields.length,
                            maxn: protocol.fields.length,
                            base: -1  // 默认设置为 -1，后续可能需要调整
                        };
                        
                        // 添加字段
                        for (var i = 0; i < protocol.fields.length; i++) {
                            var field = protocol.fields[i];
                            var f = {
                                name: field.name,
                                tag: field.index,
                                type: getTypeCode(field.type),
                                st: null
                            };
                            
                            // 如果是自定义类型，设置 st
                            if (f.type === 3) {
                                // 这里需要找到对应的类型索引，暂时设为 null
                                f.st = null;  // 后续处理
                            }
                ...
                for (var key in protocols) {
                    ...
                    if (protocol.type === 'protocol') {
                        var p = {
                            name: protocol.name,
                            tag: protocol.id,
                            p: [null, null]  // request 和 response
                        };
                        
                        // 设置 request 和 response
                        if (protocol.request) {
                            p.p[0] = findTypeByName(sproto.type, protocol.request);
                        }
                        
                        if (protocol.response) {
                            p.p[1] = findTypeByName(sproto.type, protocol.response);
                        }
                ...
                // 设置类型引用
                for (var i = 0; i < sproto.type.length; i++) {
                    var type = sproto.type[i];
                    for (var j = 0; j < type.f.length; j++) {
                        var field = type.f[j];
                        if (field.type === 3 && field.st === null) {
                            // 查找自定义类型的索引
                            var typeName = findFieldTypeName(protocols, type.name, field.name);
                            if (typeName) {
                                field.st = findTypeIndex(sproto.type, typeName);
                            }
                        }
                    }
                }
                ...
                console.error("Failed to create sproto instance from parsed protocols:", e);
                return null;
            }
        }
        ...
        // 辅助函数：查找类型索引
        function findTypeIndex(types, name) {
            for (var i = 0; i < types.length; i++) {
                if (types[i].name === name) {
                    return i;
                }
        ...
        // 辅助函数：查找字段类型名称
        function findFieldTypeName(protocols, typeName, fieldName) {
            var type = protocols[typeName];
        ...
        if (typeof data === 'string') {
            // 如果输入是字符串，先使用 parse 方法解析
            const protocols = this.parse(data);
            
            return  createFromParsedProtocols(protocols);
            
        }else if(data.buf && data.sz) {
